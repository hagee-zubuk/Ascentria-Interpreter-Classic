...
If Not rsDept.EOF Then
	...
	If tmpDeptaddr = "" Then 
		tmpDeptaddr = rsDept("InstAdrI") & " " & rsDept("address") & ", " & rsDept("City") & ", " &  rsDept("state") & ", " & rsDept("zip")
		if tmpDeptaddrG = "" then tmpDeptaddrG = rsDept("address") & ", " & rsDept("City") & ", " &  rsDept("state") & ", " & rsDept("zip")
	End If
End If
rsDept.Close
Set rsDept = Nothing 
	  ...
	  <% End If %>
		...
		function ChkComplete(IntrRate, InstRate)
		{
			var rate1 = new Boolean(true), rate2 = new Boolean(true)
				if (document.frmConfirm.txtActTFrom.value != "" && document.frmConfirm.txtActTTo.value != ""  && (document.frmConfirm.txtBilHrs.value != "" || document.frmConfirm.txtBilHrs.value > 0))
				{
