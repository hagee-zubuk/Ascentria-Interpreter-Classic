...
End Function
	...
	sqlTS = "SELECT overpayhrs, payhrs, LBconfirm, [index], totalhrs, Status, confirmed, InstID, Cfname, Clname, AStarttime, AEndtime, totalhrs, actTT, appDate FROM request_T WHERE appDate >= '" & sundate & "' AND appDate <= '" & satDate & "' AND IntrID = " & Session("UIntr") & " " & _
		"AND showintr = 1 AND status <> 2 AND Status <> 3 AND LBconfirm = 1 ORDER BY appDate ,appTimeFrom"
			...
			If rsTS("appDate") = cdate(sundate) Then
				sunTS = sunTS & "<tr bgcolor='#F5F5F5'><td><input type='hidden' name='ctr" & ctr & "' value='" & rsTS("index") & "'></td><td align='center'><nobr>" & myAct & "</td>" & _
					"<td align='center'>" & TT & "</td>" & _
				 	"<td align='center'>" & AStime & "</td>" & _ 
					"<td align='center'>" & AEtime & "</td>" & _ 
					"<td align='center'>" & rsTS("totalhrs") & "</td>" & _
					"<td align='center'>" & z_formatnumber(FPHrs, 2) & "</td>" & _
...
<html>
	<head>
		...
		{
			document.frmTS.action = 'tsheetnew.asp?tmpdate=' + strDate;
			document.frmTS.submit();
		}
		function PrevMonth()
		{
			document.frmTS.action = "tsheetnew.asp?action=1&tmpDate=" + '<%=sundate%>';
			document.frmTS.submit();	
		}
		function NextMonth()
		{
			document.frmTS.action = "tsheetnew.asp?action=2&tmpDate=" + '<%=sundate%>';
			document.frmTS.submit();	
		}
		//-->
		</script>
							...
							<table cellSpacing='0' cellPadding='0' width="100%" border='0'>
								...
								<tr>
									<td colspan='12' align='center' height='100px' valign='bottom'>
										...
										<input class='btn' type='button' value='Confirm' <%=billedna%> <%=disave%> onmouseover="this.className='hovbtn'" onmouseout="this.className='btn'" onclick='SendTS();'>
										<input class='btn' type='button' value='Print' onmouseover="this.className='hovbtn'" onmouseout="this.className='btn'" onclick='window.print();'>
