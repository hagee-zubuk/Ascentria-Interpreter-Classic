...
End Function
	...
	sqlConfirm = "SELECT * FROM Request_T WHERE [index] = " & Request("appID")
...
var desinationzip = "<%=tmpZipInstg%>";
	...
	<body onload='initMap();'>
		<form name='frmMile' method='post'>
			...
			<input type='hidden' name='selIntr' value='<%=Request("intrID")%>'>
			...
			<input type='hidden' name='ID'  value='<%=Request("appID")%>'>
			<tr>
