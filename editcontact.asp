...
End If
Function MyClass(xxx)
	If xxx = 1 Then MyClass = "Social Services"
	If xxx = 2 Then MyClass = "Private"
	If xxx = 3 Then MyClass = "Court"
	If xxx = 4 Then MyClass = "Medical"
	If xxx = 5 Then MyClass = "Legal"
End Function
...
sqlInstReq = "SELECT * FROM dept_T WHERE InstID = " & tmpInst & " ORDER BY dept"
...
Do Until rsInstReq.EOF
	...
	strDept = strDept & "if (dept == " & rsInstReq("Index") & ") " & vbCrLf & _
		...
		"document.frmAssign.txtBilZip.value = """ & rsInstReq("Bzip") &"""; " & vbCrLf & _
		"document.frmAssign.OldAddr.value = """ & tmpOLDAddr &"""; " & vbCrLf & _
		"document.frmAssign.selClass.value = """ & myClass(rsInstReq("Class")) &"""; }" & vbCrLf 
	...
	sqlReqInst = "SELECT requester_T.[index] as rpID, lname, fname, Phone, pExt, Fax, Email, prime FROM requester_T, reqdept_T WHERE  ReqID = requester_T.[index] AND DeptID = " & InstReq & " ORDER BY lname, fname"
