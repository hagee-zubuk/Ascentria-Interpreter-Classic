...
If Request.ServerVariables("REQUEST_METHOD") = "POST"  Or Request("action") = 3 Then
	If Request("radioAss") = 0 Then
		sqlReq = "SELECT request_T.InstID, IntrID, LangID, InstRate, SentReq, Processed, Status, DeptID, request_T.[index]," & _
			" clname, cfname, appDate, appTimeFrom, appTimeTo, astarttime, aendtime, Billable, IntrRate, comment " & _
	...
	Else
		sqlReq = "SELECT request_T.InstID, IntrID, LangID, InstRate, SentReq, Processed, Status, DeptID, request_T.[index]," & _
			" clname, cfname, appDate, appTimeFrom, appTimeTo, astarttime, aendtime, Billable, IntrRate, comment " & _
			"FROM request_T, institution_T, language_T, requester_T, dept_T " & _
		"WHERE request_T.InstID = institution_T.[index] " & _
		"AND LangId = language_T.[index] " & _
		"AND request_T.DeptId = dept_T.[index] " & _
		"AND (IntrId = -1 OR IntrId = 0)" & _
		"AND ReqID = requester_T.[index]"
		radioAss = ""
		radioUnass = "checked"
	End If
	...
	If Request("sort") <> "" Then
		...
		If Request("sort") = 11 Then sqlReq = sqlReq & " ORDER BY request_T.InstRate"
		If Request("sort") = 14 Then sqlReq = sqlReq & " ORDER BY request_T.IntrRate"
...
If Not rsReq.EOF Then
	Do Until rsReq.EOF
		...
		If Cint(Request.Cookies("LBUSERTYPE")) <> 4 Then 
			...
			If Cint(Request.Cookies("LBUSERTYPE")) = 1 Then
				...
				csvTBL = csvTBL & rsReq("Index") & "," & tmpIname & myDept & "," & tmpSalita & "," & rsReq("clname") & ", " & rsReq("cfname") & _
					"," & tmpInName & "," & rsReq("appDate") & "," & Z_FormatTime(rsReq("appTimeFrom")) & "," & Z_FormatTime(rsReq("appTimeTo")) & _
		...
		Else
				strtbl = strtbl & "<tr bgcolor='" & kulay & "'>" & vbCrLf & _ 
				"<td class='tblgrn2' width='10px'>" & Stat & "</td>" & vbCrLf & _
				"<td class='tblgrn2' ><input type='hidden' name='ID" & x & "' value='" & rsReq("request_T.[index]") & "'><a class='link2' href='reqconfirm.asp?ID=" & rsReq("request_T.[index]") & "'><b>" & rsReq("request_T.[index]") & "</b></a></td>" & vbCrLf & _
				"<td class='tblgrn2' ><nobr>" & tmpIname & myDept & "</td>" & vbCrLf & _
				"<td class='tblgrn2' >" & tmpSalita & "</td>" & vbCrLf & _
				"<td class='tblgrn2' >" & tmpInName & "</td>" & vbCrLf & _
				"<td class='tblgrn2' >" & rsReq("appDate") & "</td>" & vbCrLf & _
