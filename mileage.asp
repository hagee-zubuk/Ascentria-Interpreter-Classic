...
<%
	...
	sqlTS = "SELECT * FROM request_T WHERE request_T.[instID] <> 479 AND Month(appDate) = " & tmpMonth & " AND Year(appDate) = " & tmpYear & " AND IntrID = " & Session("UIntr") & " " & _
	...
	If Not rsTS.EOF Then
		Do Until rsTS.EOF
			...
			strMile = strMile & "<tr bgcolor='#F5F5F5'><td align='center'><input type='hidden' name='ctr" & ctr & "' value='" & rsTS("index") & "'>" & rsTS("appdate") & "</td>" & _
				...
				"<td align='center'>" & rsTS("RealM") & "</td>" & _
			 	"<td align='center'>" & tmpAMT & "</td>" & _
				"<td align='center'>$" & Z_FormatNumber(Z_CZero(rsTS("toll")), 2) & "</td>" & _
										...
										<form name='frmTS' method='POST'>
											<table border='0' cellpadding='1' cellspacing='2' width='75%'>
												<tr>
													...
													<td align='center' class='tblgrn'>Payable Mileage</td>
